...
class ImageMessageItem(
    ...
    override fun isItemType(item: UIMessage): Boolean {
        return item.messageType == UIMessage.TYPE_IMAGE
    }
    ...
    override fun ListItem(item: UIMessage) {
        if (item.originType == UIMessage.ORIGIN_TYPE_RECEIVING) {
            ReceivingMessage(
                ...
                ImageContent(path = item.image?.thumb, backgroundColor = MaterialTheme.colors.surface)
            }
        } else {
            SendingMessage(item, onContentClicked = { onContentClicked(it, item)}) {
                ImageContent(path = item.image?.thumb, backgroundColor = MaterialTheme.colors.primary)
            }
        }
    }
}